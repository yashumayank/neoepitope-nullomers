...
awk '{if(NR==FNR){a[$1]=$2;next}{if($0~/^>.*/){count=0;seqlen=length(seq)-15;for(x=1;x<=seqlen;x++){y=substr(seq,x,16);if(y in a){if(a[y]<300){print y;count++;x+=10}}};seq=""; print count > "test"}else{seq=seq $1}}}END{count = 0;seqlen=length(seq)-15;for(x=1;x<=seqlen;x++){y=substr(seq,x,16);if(a[y]!=1){print y;count++}};print count > "test"}' TE_nullomers_freq.tsv TE_seqs_named.fasta| sort| uniq > TE-nullomers.tsv
